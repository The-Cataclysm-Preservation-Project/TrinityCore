...
void AuthSession::CheckIpCallback(PreparedQueryResult result)
{
    if (result)
    {
        ...
        if (banned)
        {
            ...
            SendPacket(pkt);
            TC_LOG_DEBUG("session", "[AuthSession::CheckIpCallback] Banned ip '%s:%u' tries to login!", GetRemoteIpAddress().to_string().c_str(), GetRemotePort());
            return;
        }
    }
...
bool AuthSession::HandleLogonChallenge()
{
    ...
    TC_LOG_DEBUG("server.authserver", "[AuthChallenge] '%s'", login.data());
...
void AuthSession::LogonChallengeCallback(PreparedQueryResult result)
{
    ...
    {
        TC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '%s' is locked to IP - '%s' is logging in from '%s'", _accountInfo.Login.c_str(), _accountInfo.LastIP.c_str(), ipAddress.data());
    ...
    else
    {
        ...
        {
            TC_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '%s' is locked to country: '%s' Player country is '%s'", _accountInfo.Login.c_str(), _accountInfo.LockCountry.c_str(), _ipCountry.data());
    ...
    if (_accountInfo.IsBanned)
    {
        if (_accountInfo.IsPermanenetlyBanned)
        {
            pkt << uint8(WOW_FAIL_BANNED);
            SendPacket(pkt);
            TC_LOG_INFO("server.authserver.banned", "'%s:%u' [AuthChallenge] Banned account %s tried to login!", ipAddress.c_str(), port, _accountInfo.Login.c_str());
            return;
        }
        else
        {
            ...
            SendPacket(pkt);
            TC_LOG_INFO("server.authserver.banned", "'%s:%u' [AuthChallenge] Temporarily banned account %s tried to login!", ipAddress.c_str(), port, _accountInfo.Login.c_str());
            return;
        }
    }
    ...
    if (AuthHelper::IsAcceptedClientBuild(_build))
    {
        ...
        TC_LOG_DEBUG("server.authserver", "'%s:%u' [AuthChallenge] account %s is using '%s' locale (%u)",
            ipAddress.c_str(), port, _accountInfo.Login.c_str(), localeNames[_locale], uint32(_locale));
...
bool AuthSession::HandleLogonProof()
{
    ...
    if (std::optional<SessionKey> K = _srp6->VerifyChallengeResponse(logonProof->A, logonProof->clientM))
    {
        ...
        TC_LOG_DEBUG("server.authserver", "'%s:%u' User '%s' successfully authenticated", GetRemoteIpAddress().to_string().c_str(), GetRemotePort(), _accountInfo.Login.c_str());
    ...
    else
    {
        ...
        TC_LOG_INFO("server.authserver.hack", "'%s:%u' [AuthChallenge] account %s tried to login with invalid password!",
        ...
        if (MaxWrongPassCount > 0)
        {
            ...
            if (++_accountInfo.FailedLogins >= MaxWrongPassCount)
            {
                ...
                if (WrongPassBanType)
                {
                    ...
                    TC_LOG_DEBUG("server.authserver", "'%s:%u' [AuthChallenge] account %s got banned for '%u' seconds because it failed to authenticate '%u' times",
                ...
                else
                {
                    ...
                    TC_LOG_DEBUG("server.authserver", "'%s:%u' [AuthChallenge] IP got banned for '%u' seconds because account %s failed to authenticate '%u' times",
...
bool AuthSession::HandleReconnectChallenge()
{
    ...
    std::string_view login(challenge->I, challenge->I_len);
    TC_LOG_DEBUG("server.authserver", "[ReconnectChallenge] '%s'", login.data());
...
bool AuthSession::HandleReconnectProof()
{
    ...
    {
        TC_LOG_ERROR("server.authserver.hack", "'%s:%u' [ERROR] user %s tried to login, but session is invalid.",
            GetRemoteIpAddress().to_string().c_str(), GetRemotePort(), _accountInfo.Login.c_str());
        return false;
    }
}
...
void AuthSession::RealmListCallback(PreparedQueryResult result)
{
    ...
    for (GruntRealmList::RealmMap::value_type const& i : sGruntRealmList->GetRealms())
    {
...
void AuthSession::SetTimeout()
{
    ...
    _timeout.async_wait([selfRef = weak_from_this()](boost::system::error_code const& error)
    {
        ...
        TC_LOG_DEBUG("server.authserver", "%s:%u session timed out.", self->GetRemoteIpAddress().to_string().c_str(), self->GetRemotePort());
        self->CloseSocket();
    });
}
