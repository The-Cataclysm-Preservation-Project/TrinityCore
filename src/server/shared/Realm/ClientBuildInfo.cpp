...
void LoadBuildInfo()
{
    ...
    if (QueryResult result = LoginDatabase.Query("SELECT `build`, `platform`, `executableHash` FROM `build_executable_hash`"))
    {
        do
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "ClientBuild::LoadBuildInfo: Unknown `build` %u in `build_executable_hash` - missing from `build_info`, skipped.", build);
                continue;
            }
            ...
            {
                TC_LOG_ERROR("sql.sql", "ClientBuild::LoadBuildInfo: Invalid platform %s for `build` %u in `build_executable_hash`, skipped.", platform.data(), build);
                continue;
            }
